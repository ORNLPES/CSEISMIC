function pull_and_save_energy_storage_SOCs()
%[] = pull_and_save_energy_storage_SOCs()
% pulls and saves SOC data into files for optimization.

global home;

%% load system measurement data %%%%%%
try 
    cd(fullfile(home,'System Data','Measurements'))
    [SOCs] = load_measurements_SOC();
catch Me
     create_error_notification(home,Me.message,'pull and save energy storage SOCs_extract SOC from mesaured data.txt'); 
end;    
...
try 
   ...
   cd(fullfile(home));
     ...
     create_error_notification(home,Me.message,'pull and save energy storage SOCs_saving SOC data.txt'); 
end; 

%Battery #	Bus #	Voltage (V)	Power Rating (kVA)	Energy Rating (kWh)	Ramp Rate (kW/s)	Max Power Factor (leading)	Max Power factor (lagging)	Min SOC (%)	Max SOC(%)	Charging Efficiency	Discharging Efficiency	Start SOC(%)	End SOC(%)
%1	2	480	50	200	8000	0.6	-0.6	25	95	0.95	0.95	50	50
