function get_system_measurements(handles)
%[] = get_system_measurements(handles)
% pulls system measurements from SCADA
...
try
    ...
    create_error_notification(home,Me.message,'on grid mode_get system measurements_opening file.txt');
end

%% pull optimization variables and load data
try
    ...
    create_error_notification(home,Me.message,'on grid mode_get system measurements_opening file2.txt');
end
      
try
    %% send requests to SCADA for measuremnts
    ...
    create_error_notification(home,Me.message,'on grid mode_get system measurements_sending request.txt');
end;
...
try
    ...
    [IEDname,Measurementname,measurement_actual] = receive_request_fullmeasurements(handles.program_status, home, SCADA_receive)
        ...
        create_error_notification(home,Me.message,'on grid mode_get system measurements_getting confirmation.txt');
end;

try
    % break apart devices and measurements
    [IEDname_array] = strsplit(IEDname{1}',',');
    [Measurementname_array] = strsplit(Measurementname{1}',',');

catch Me
     create_error_notification(home,Me.message,'on grid mode_get system measurements_parsing returned data.txt');
...
try
   ...
   %%
    ...
    fclose(fid)
            ...
            create_error_notification(home,Me.message,'on grid mode_get system measurements_saving measurements.txt');
end;
